...
PANEL_PORT = "21524"
SUB_PORT = "2096"
SECRET_PATH = "rNsOideTrxjP1O05fX" # Твой путь из сообщения
...
def add_user_to_panel(user_id):
    ...
    client_data = {
        "id": u_uuid, "email": f"u_{user_id}", "totalGB": 0, "expiryTime": 0, 
        "enable": True, "tgId": str(user_id), "subId": u_sub_id
    }
...
async def check_payment(callback: CallbackQuery):
    ...
    if payment.status == 'succeeded':
        ...
        await callback.message.edit_text(f"✅ Оплачено! Подписка до {time.strftime('%d.%m.%Y', time.localtime(expiry))}\n\nТвоя ссылка:\n{hcode(get_vpn_link(u_id))}", parse_mode="HTML")
    else:
...
async def admin_give(message: types.Message, command: CommandObject):
    ...
    conn.close()
    if u:
        expiry = await activate_user_logic(u[0], "Admin", days, is_days=True)
        await message.answer(f"✅ Выдано @{target}")
        try: await bot.send_message(u[0], f"🎁 Доступ выдан!\n{hcode(get_vpn_link(u[0]))}", parse_mode="HTML")
        except: pass
    else: await message.answer("❌ Юзер не в базе")
...
async def about_menu(callback: CallbackQuery):
    ...
    await callback.message.edit_text("📖 Информация:", reply_markup=InlineKeyboardMarkup(inline_keyboard=btns), parse_mode="HTML")
