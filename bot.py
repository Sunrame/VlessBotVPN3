...
def fmt_dt(ts: int, fmt: str = "%d.%m.%Y %H:%M") -> str:
    """Форматировать unix-timestamp в московское время."""
    if not ts:
        return "∞"
...
# FSM для интерактивных действий /check
class CheckActionState(StatesGroup):
    waiting_days_input   = State()   # произвольное кол-во дней
    waiting_hwid_input   = State()   # произвольный лимит устройств
    waiting_expire_input = State()   # установить конкретную дату
...
async def is_admin_sale_notify(admin_id: int) -> bool:
    """Включены ли уведомления о покупках для этого админа."""
    async with pool.acquire() as conn:
        row = await conn.fetchrow("SELECT sale_notify FROM admin_settings WHERE admin_id=$1", admin_id)
        if row is None:
            return True  # по умолчанию включено
        return row["sale_notify"] if row["sale_notify"] is not None else True
...
def format_key_message(user: dict) -> str:
    ...
    lines = [f"🗓 Подписка до: <b>{date_str} (МСК)</b>", "", "━━━━━━━━━━━━━━━━━━━━"]
    if sub_url:
        lines += [
...
async def notify_admins_sale(u_id: int, username: str, tariff_name: str,
                              days: int, price: float, is_trial: bool):
    """Уведомить всех админов о покупке."""
...
def _check_kb(user_id: int, hwid: int) -> InlineKeyboardMarkup:
    ...
    # Текущий hwid — быстрые пресеты
    preset_row = []
    for limit, label in HWID_OPTIONS.items():
        mark = "✅" if limit == hwid else ""
        preset_row.append(
            InlineKeyboardButton(text=f"{mark}{label}", callback_data=f"setlim_{user_id}_{limit}")
        )
        if len(preset_row) == 3:
...
async def check_payment(cb: CallbackQuery):
    ...
    await notify_admins_sale(u_id, uname, tariff_name, days, price, is_trial)
...
async def profile_tab(cb: CallbackQuery):
    ...
    if user and parse_dt(user.get("expireAt")) > now and user.get("status") == "ACTIVE":
        ...
        text = (
            f"👤 <b>Профиль</b>\n\n"
            f"✅ Подписка активна · до <b>{date_str} МСК</b>\n"
            f"⏳ Осталось: <b>{days_left} дн.</b>\n"
...
async def support_to_main(cb: CallbackQuery, state: FSMContext):
    await cb.answer()
    await state.clear()
    await cb.message.edit_text(
        f"🌏 {hbold('TrubaVPN')} — быстрый и надёжный VPN.",
...
async def ticket_view(cb: CallbackQuery):
    ...
    lines = [f"🎫 <b>Тикет #{ticket_id}</b>", f"👤 {uname}", f"📅 {created} МСК",
...
# ─────────────────────────────────────────────
#  /check — расширенные интерактивные действия
# ─────────────────────────────────────────────
async def _send_check(message_or_cb, user_id: int):
    """Общая функция отображения карточки /check."""
    now = int(time.time())
    async with pool.acquire() as conn:
...
async def admin_check(message: types.Message, command: CommandObject):
    ...
    await _send_check(message, db_row["user_id"])

# --- Добавить дни ---
@router.callback_query(F.data.startswith("ca_adddays_"))
async def ca_adddays(cb: CallbackQuery, state: FSMContext):
    await cb.answer()
    if cb.from_user.id not in ADMIN_IDS: return
    user_id = int(cb.data.removeprefix("ca_adddays_"))
    await state.set_state(CheckActionState.waiting_days_input)
    await state.update_data(ca_user_id=user_id, ca_action="add")
...
async def ca_subdays(cb: CallbackQuery, state: FSMContext):
    await cb.answer()
    if cb.from_user.id not in ADMIN_IDS: return
    user_id = int(cb.data.removeprefix("ca_subdays_"))
    await state.set_state(CheckActionState.waiting_days_input)
    await state.update_data(ca_user_id=user_id, ca_action="sub")
    await cb.message.answer(
        f"➖ <b>Отнять дни</b> у ID:{user_id}\n\nВведите количество дней (например: <code>7</code>):\n/cancel — отмена",
        parse_mode="HTML",
    )

# --- Установить дату ---
@router.callback_query(F.data.startswith("ca_setdate_"))
async def ca_setdate(cb: CallbackQuery, state: FSMContext):
    await cb.answer()
    if cb.from_user.id not in ADMIN_IDS: return
    user_id = int(cb.data.removeprefix("ca_setdate_"))
    await state.set_state(CheckActionState.waiting_expire_input)
    await state.update_data(ca_user_id=user_id)
    await cb.message.answer(
        f"📅 <b>Установить дату истечения</b> для ID:{user_id}\n\n"
        f"Введите дату в формате <code>ДД.ММ.ГГГГ</code> (МСК):\n/cancel — отмена",
        parse_mode="HTML",
    )

# --- Установить кол-во устройств вручную ---
@router.callback_query(F.data.startswith("ca_sethwid_"))
async def ca_sethwid(cb: CallbackQuery, state: FSMContext):
    await cb.answer()
    if cb.from_user.id not in ADMIN_IDS: return
    user_id = int(cb.data.removeprefix("ca_sethwid_"))
    await state.set_state(CheckActionState.waiting_hwid_input)
    await state.update_data(ca_user_id=user_id)
    await cb.message.answer(
        f"📱 <b>Установить лимит устройств</b> для ID:{user_id}\n\n"
        f"Введите число (0 = без лимита, макс. 100):\n/cancel — отмена",
        parse_mode="HTML",
    )

@router.message(Command("cancel"), CheckActionState.waiting_days_input)
@router.message(Command("cancel"), CheckActionState.waiting_hwid_input)
@router.message(Command("cancel"), CheckActionState.waiting_expire_input)
async def ca_cancel(message: types.Message, state: FSMContext):
    await state.clear()
    await message.answer("Отменено.")

@router.message(CheckActionState.waiting_days_input)
async def ca_days_handler(message: types.Message, state: FSMContext):
    if message.from_user.id not in ADMIN_IDS: return
    text = message.text.strip()
    if not text.isdigit() or int(text) == 0:
        await message.answer("❌ Введите целое положительное число."); return
    data    = await state.get_data()
    user_id = data["ca_user_id"]
    action  = data["ca_action"]
    days    = int(text)
    await state.clear()
    ...
    if action == "add":
        ...
        verb       = f"➕ добавлено <b>{days}</b> дн."
    else:
        new_expire = current - timedelta(days=days)
        if new_expire < now:
            new_expire = now + timedelta(minutes=1)
        verb = f"➖ отнято <b>{days}</b> дн."

    new_expire_str = new_expire.strftime("%Y-%m-%dT%H:%M:%S.000Z")
    result = await remna_update_user(remna["uuid"], {"expireAt": new_expire_str})
    if not result:
        await message.answer("❌ Ошибка обновления."); return

    expire_fmt = fmt_dt(int(new_expire.timestamp()))
    await message.answer(
        f"✅ ID:{user_id} — {verb}\n📅 Новая дата: <b>{expire_fmt} МСК</b>",
        parse_mode="HTML",
    )
    await _send_check(message, user_id)

@router.message(CheckActionState.waiting_expire_input)
async def ca_expire_handler(message: types.Message, state: FSMContext):
    if message.from_user.id not in ADMIN_IDS: return
    text = message.text.strip()
    data = await state.get_data()
    user_id = data["ca_user_id"]
    await state.clear()
    try:
        dt_msk     = datetime.strptime(text, "%d.%m.%Y").replace(
    ...
    await message.answer(
        f"✅ ID:{user_id} — дата истечения установлена: <b>{text} 23:59 МСК</b>",
        parse_mode="HTML",
    )
    await _send_check(message, user_id)

@router.message(CheckActionState.waiting_hwid_input)
async def ca_hwid_handler(message: types.Message, state: FSMContext):
    if message.from_user.id not in ADMIN_IDS: return
    text = message.text.strip()
    data = await state.get_data()
    user_id = data["ca_user_id"]
...
# setlim_ (быстрые пресеты устройств из _check_kb)
...
async def admin_help(message: types.Message):
    ...
    await message.answer(
        ...
        "<code>/check username|id</code> — инфо + действия\n"
