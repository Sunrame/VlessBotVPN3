...
# Формируем базовый URL для API (строго по твоим данным)
...
async def activate_user_logic(user_id, plan, amount, is_days=False):
    ...
    # Если подписка еще жива, прибавляем к ней, если нет — от текущего момента
    ...
    # Логика рефералов (если первый раз активирует)
...
def get_vpn_link(user_id):
    ...
    return f"vless://{u_uuid}@{SERVER_IP}:443?{params}#TrubaVPN_{user_id}"
