...
# --- КОНФИГУРАЦИЯ (СВЕРЕНО СО СКРИНШОТАМИ) ---
...
def add_user_to_panel(user_id):
    ...
    # Генерируем UUID и subId (токен подписки)
...
def get_vpn_link(user_id):
    # Ссылка подписки строится на основе subId
    u_sub_id = str(uuid.uuid5(uuid.NAMESPACE_DNS, f"sub_{user_id}"))[:12]
...
async def cmd_start(message: types.Message, command: CommandObject):
    ...
    user_name = (message.from_user.username or "none").lower()
    conn = sqlite3.connect('users.db')
...
async def admin_take(message: types.Message, command: CommandObject):
    ...
    conn.commit(); conn.close()
    await message.answer(f"⛔ Подписка @{target} удалена.")
