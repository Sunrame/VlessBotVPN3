...
# --- КОНФИГУРАЦИЯ ПАНЕЛИ (Настройки Xray) ---
SERVER_IP = "213.176.94.201"
PANEL_PORT = "21524"
SUB_PORT = "2096" # ПОРТ ПОДПИСКИ
SECRET_PATH = "rNsOideTrxjP1O05fX" # ИСПОЛЬЗУЕМ ТВОЙ ПУТЬ
...
# --- API ПАНЕЛИ (С ПОДДЕРЖКОЙ ПОДПИСКИ) ---
def login_to_panel():
    try:
        ...
        logging.error(f"Критическая ошибка API Login: {e}")
        return False
...
def add_user_to_panel(user_id):
    ...
    # Генерируем стабильный subId для подписки
    user_sub_id = str(uuid.uuid5(uuid.NAMESPACE_DNS, f"sub_truba_{user_id}"))[:12]
    ...
    client_data = {
        ...
        "enable": True,
        "tgId": str(user_id),
        "subId": user_sub_id # Передаем subId в панель
    }
    ...
    try:
        ...
        if result.get("success") or "already exists" in result.get("msg", ""):
            logging.info(f"Клиент {user_id} успешно обработан в панели.")
            return True
        logging.error(f"Панель отклонила создание: {result.get('msg')}")
        return False
    except Exception as e:
        logging.error(f"Ошибка при вызове addClient: {e}")
        return False
...
async def activate_user_logic(user_id, plan, amount, is_days=False):
    ...
    if not is_days and row and not row[2] and row[1]:
        ...
        if friend_count and friend_count[0] >= 5:
            forever = now + 3153600000 
...
# --- НОВАЯ ФУНКЦИЯ ВЫДАЧИ ПОДПИСКИ ---
def get_vpn_link(user_id):
    # Генерируем тот же subId, что и при создании клиента
    user_sub_id = str(uuid.uuid5(uuid.NAMESPACE_DNS, f"sub_truba_{user_id}"))[:12]
    # Формируем ссылку на подписку
    return f"http://{SERVER_IP}:{SUB_PORT}/sub/{user_sub_id}"

# --- ОБРАБОТЧИКИ ТЕЛЕГРАМ (БЕЗ ИЗМЕНЕНИЙ) ---
...
async def admin_give(message: types.Message, command: CommandObject):
    ...
    if row:
        ...
        await message.answer(f"✅ Выдано @{target} на {days} дней.")
        try:
            await bot.send_message(row[0], f"🎁 Доступ выдан!\n{hcode(get_vpn_link(row[0]))}", parse_mode="HTML")
        except:
            pass
    else:
        await message.answer("❌ Юзер не найден в базе данных бота")

@router.message(Command("take"))
async def admin_take(message: types.Message, command: CommandObject):
    if message.from_user.id not in ADMINS:
        return
    target = command.args.replace("@", "").strip().lower() if command.args else ""
    conn = sqlite3.connect('users.db')
    cursor = conn.cursor()
    ...
    conn.close()
    if success:
        await message.answer(f"⛔ Подписка @{target} полностью удалена")
    else:
...
async def about_menu(callback: CallbackQuery):
    ...
    await callback.message.edit_text("📖 <b>Информация и помощь:</b>", reply_markup=InlineKeyboardMarkup(inline_keyboard=btns), parse_mode="HTML")
...
async def show_ref(callback: CallbackQuery):
    ...
    await callback.message.edit_text(f"🤝 <b>Пригласи 5 друзей и получи Вечный Премиум!</b>\n\n👥 Приглашено друзей: {friends}/5\n🔗 Твоя ссылка:\n{hcode(link)}", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="⬅️ Назад", callback_data="to_main")]]), parse_mode="HTML")

@router.callback_query(F.data == "profile")
async def show_profile(callback: CallbackQuery):
    d = get_user_data(callback.from_user.id)
    is_active = d and d[0] > int(time.time())
    text = f"👤 <b>Ваш профиль</b>\n\n📅 Срок до: {time.strftime('%d.%m.%Y', time.localtime(d[0])) if is_active else '❌ Подписка не активна'}"
    if is_active:
