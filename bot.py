...
# Базовый URL подписки — может быть на другом порту, чем панель.
# Пример: https://213.176.94.201:2096
# Если не задан — используется PANEL_URL (старое поведение).
SUB_BASE_URL = os.environ.get("SUB_BASE_URL", "").rstrip("/") or PANEL_URL
SUB_PATH     = os.environ.get("SUB_PATH", "/sub/")   # корневой путь из настроек панели
...
def init_db():
    with db_conn() as conn:
        ...
        try:
            ...
            pass
        try:
            conn.execute("ALTER TABLE users ADD COLUMN sub_id TEXT")
        except Exception:
            pass
...
def build_subscription_url(sub_id: str) -> str:
    """URL подписки по subId на правильном порту (SUB_BASE_URL:2096)."""
    path = SUB_PATH if SUB_PATH.endswith("/") else SUB_PATH + "/"
    return f"{SUB_BASE_URL}{path}{sub_id}"
...
def format_key_message(expiry: int, vless_link: str, sub_url: str | None) -> str:
    ...
    if vless_link.startswith("PANEL_ERROR_"):
        return (
            ...
            f"✍️ Напишите в поддержку: {SUPPORT_CONTACT}"
        )
...
def _sync_create_client(user_id: int, days: int, limit_ip: int = 0) -> tuple[str | None, str | None, str | None]:
    ...
    sub_id     = uuid.uuid4().hex[:12]   # сохраняем, чтобы использовать в ссылке
    client_obj = {
        "id":         client_id,
        "email":      email,
...
async def activate_subscription(user_id: int, days: int, limit_ip: int = 0):
    ...
    with db_conn() as conn:
        ...
        if client_uuid:
            ...
            if ok:
                # при продлении subId не меняется — используем сохранённый
                if saved_sub_id:
                    sub_url = build_subscription_url(saved_sub_id)
            else:
        ...
        if not client_uuid:
            ...
            if not vless_link:
                ...
                # извлекаем subId из только что построенного sub_url
                saved_sub_id = sub_url.rstrip("/").split("/")[-1] if sub_url else None
            token = vless_link
...
async def profile_tab(cb: CallbackQuery):
    ...
    if row and row["expiry_date"] > now:
        ...
        if row["client_uuid"] and row["sub_id"]:
            sub_url  = build_subscription_url(row["sub_id"])
...
async def broadcast_start(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        "📢 <b>Рассылка</b>\n\n"
        "✍️ Введите текст сообщения (HTML поддерживается).\n"
        "/cancel — отмена.",
        parse_mode="HTML",
    )
...
async def broadcast_preview(message: types.Message, state: FSMContext):
    # Сохраняем текст в FSM-хранилище (не в dp.storage напрямую)
...
async def broadcast_confirm(cb: CallbackQuery, state: FSMContext):
    ...
    # Берём текст из FSMContext — надёжно и без костылей
    data = await state.get_data()
